...
describe('Posts and Comments test suite', () => {
    ...
    test('Fail to create a post without required fields', async () => {
        const response = await request(app).post('/posts').send({ title: 'Missing Fields' });
        expect(response.statusCode).toBe(400);
        expect(response.body.message).toBe('All fields are required');
    });

    test('Get all posts after creating one', async () => {
        await postModel.create({ title: 'Test Post', content: 'Test Content', senderId: 'TestSenderId' });
        const response = await request(app).get('/posts');
        expect(response.statusCode).toBe(200);
    ...
    test('Get a post by ID successfully', async () => {
        const post = await postModel.create({ title: 'Test Post', content: 'Test Content', senderId: 'TestSenderId' });
    ...
    test('Successfully get comment by id', async () => {
        const comment = await commectModel.findOne();
